...
y_points = 400 # f samples  (-1 for not down sampling, keep origional) 
chunk_t = datetime.timedelta(minutes=15)
chop_off = True # chop every **interger** 15 minutes [00:15,00:30,00:45....]


folder = '/HDD/mnedal/LOFAR_School/data/BF/'
sub_folders = [name for name in os.listdir(folder) if os.path.isfile(os.path.join(folder, name))]

for SASID in sub_folders:
    h5dir = folder+SASID
    #os.chdir(h5dir)                 # the dir contains the h5
    #out_dir_base = folder+SASID+'/' # should be absolute dir starting from '/'
    out_dir_base = folder
    
    #for fname_DS in glob.glob(h5dir+'/*.h5'):
    for fname_DS in glob.glob(folder+'*.h5'):
